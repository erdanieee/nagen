...
-R $referenceGenome \
-V $outputDir/$familyId'.2.vcf.gz' \
--select-type-to-include SNP \
-O $outputDir/$familyId'.snvs.vcf.gz'
...
-R $referenceGenome \
-V $outputDir/$familyId'.snvs.vcf.gz' \
-filter "QD < 2.0" \
...
-O $outputDir/$familyId'.snvs.labeled.vcf.gz'
...
-O $outputDir/$familyId'.indels.vcf.gz'
...
/opt/gatk-4.0.0.0/gatk VariantFiltration \
-R $referenceGenome \
-V $outputDir/$familyId'.indels.vcf.gz' \
-filter "QD < 2.0" \
--filter-name "QualByDepth" \
-filter "FS > 200.0" \
--filter-name "FisherStrand" \
...
-filter "SOR > 10.0" \
--filter-name "StrandOddsRatio" \
-O $outputDir/$familyId'.indels.labeled.vcf.gz'
...
-R $referenceGenome \
-I $outputDir/$familyId'.snvs.labeled.vcf.gz' \
-I $outputDir/$familyId'.indels.labeled.vcf.gz' \
-O $outputDir/$familyId'.labeled.vcf.gz'
